...
class Currency {
	...
	protected $options_currencies = array();
	...
	public function setCacheCurrencies()
	{
		...
		$this->currencies = Cache::rememberForever('ado.currency', function() use($db)
		{
			$cache      = array();
	...
	public function getCurrencies($type='all'){
		 if($type=='all'){
			 $currencies = $this->currencies;
		 }elseif($type=='options'){
			 $currencies = $this->currencies;
			$this->options_currencies =  Cache::rememberForever('ado.currency_options', function() use($currencies)
			{
				$cache      = array();
				foreach ($currencies as $currency)
				{
					$cache[$currency['code']] = $currency['code'];
				}
				return $cache;
			});
			 $currencies = $this->options_currencies;
		 }
		return $currencies;
	}
}
