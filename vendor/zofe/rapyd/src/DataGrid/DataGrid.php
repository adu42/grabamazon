...
class DataGrid extends DataSet
{
    ...
    public function form($url,$btnLabel='',$position='BL'){
        $saveButtonPosition = $position?:(Config::get('rapyd.data_edit.button_position.save') != '' ? Config::get('rapyd.data_edit.button_position.save') : 'BL');
        $form_attr = array('url' => $url, 'class' => "form-horizontal", 'role' => "form", 'method' => 'post');
        $formOpen = Form::open($form_attr);
        $formClose = Form::close();
        $btnLabel = empty($btnLabel)?trans('rapyd::rapyd.save'):$btnLabel;
